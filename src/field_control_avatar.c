...
int ProcessPlayerFieldInput(struct FieldInput *input)
{
    ...
    if (input->pressedSelectButton && UseRegisteredKeyItemOnField() == TRUE)
        return TRUE;
    if (input->pressedRButton && TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_MACH_BIKE | PLAYER_AVATAR_FLAG_UNI_BIKE))
    {
        if (gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_MACH_BIKE)
        {
            gPlayerAvatar.flags -= PLAYER_AVATAR_FLAG_MACH_BIKE;
            gPlayerAvatar.flags += PLAYER_AVATAR_FLAG_UNI_BIKE;
            SetPlayerAvatarTransitionFlags(PLAYER_AVATAR_FLAG_UNI_BIKE);
            PlaySE(SE_BIKE_HOP);
        }
        else
        {
