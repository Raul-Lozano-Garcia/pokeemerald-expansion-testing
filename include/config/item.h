...
#define I_SITRUS_BERRY_HEAL             FALSE  // In Gen4+, Sitrus Berry was changed from healing 30 HP to healing 25% of Max HP.
#define I_VITAMIN_EV_CAP                GEN_LATEST  // In Gen8+, the Vitamins no longer have a cap of 100 EV per stat.
#define I_BERRY_EV_JUMP                 FALSE  // In Gen4 only, EV-lowering Berries lower a stat's EV to 100 if it is above 100.
...
#define I_TYPE_BOOST_POWER              FALSE  // In Gen4+, all regular type boosting held items had their power increased from 10% to 20%. eg. Charcoal
#define I_SELL_VALUE_FRACTION           FALSE  // In Gen9+, items sell for 1/4 of their value instead of 1/2.
#define I_PRICE                         FALSE  // Some items have varied in value across generations.
#define I_BERRY_PRICE                   FALSE       // Since Berries have become unplantable (Gen8+), their price has gone up.
#define I_POWER_ITEM_BOOST              GEN_LATEST  // In Gen7+, Power Items grant 8 EVs instead of 4 EVs.
#define I_PREMIER_BALL_BONUS            FALSE  // In LGPE onwards (Gen8+ here), you are given a Premier Ball for every 10 Poké Balls of any type and in the same purchase. Previously, it only applied to regular Poké Balls and only 1 could be obtained per purchase.
